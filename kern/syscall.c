...
sys_exofork(void)
{
	...
	retcode = env_alloc(&new_env, curenv->env_id);
	if (retcode < 0 ) {
		cprintf("sys_exofork error: %e", retcode);
		return retcode;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if ((envid2env(srcenvid, &src_env, 1) < 0) ||
		(envid2env(dstenvid, &des_env, 1) < 0)) {
		return -E_BAD_ENV;
	}
	...
	if (page_insert(des_env->env_pgdir, pp, dstva, perm) < 0) {
		return -E_NO_MEM;
	}
	return 0;
}
